...
class AUNet_Res(nn.Module):
    def __init__(self, n_channels, n_classes, pretrained_model, learned_bilinear=False):
        super(AUNet_Res, self).__init__()
        self.n_channels=n_channels
        self.n_classes=n_classes
        ...
        self.up_block1 = Attention_Up_block(2048, 1024, 1024, learned_bilinear)
        self.up_block2 = Attention_Up_block(1024, 512, 512, learned_bilinear)
        self.up_block3 = Attention_Up_block(512, 256, 256, learned_bilinear)
        #self.up_block4 = Attention_Up_block(128, 64, 64, learned_bilinear)
    ...
    def forward(self, x):
        ...
        x = self.up_block1(context_blocks[1], context_blocks[0])
        x = self.up_block2(context_blocks[2], x)
        x = self.up_block3(context_blocks[3], x)
...
#     model = AUNet_Res(n_channels=3, n_classes=2, pretrained_model=True, learned_bilinear=False)
#     model.eval()
