...
def test(ori_img, mask_img, save_path,Net,line_color =(0,0,255), line_width = 5, add_mask= False):
    parser = argparse.ArgumentParser()
    parser.add_argument("--image_size",type=int,default=512,help="")
    parser.add_argument("--class_num", type=int, default=2, help="")
    parser.add_argument("--weights_path", type=str, default="./save/6.pth", help="")
    ...
    Net = Net(n_channels=3, n_classes=CLASS_NUM)
    dir_path =  save_path 
    ...
    with torch.no_grad():
        Net.eval()
        output,cla,feas = Net(image)
        output = output[0]
        cla = cla[0]
        _, predicted = torch.max(output.data, 0)
        predicted = predicted.cpu().numpy().astype(np.uint8)
        outs = predicted*255
        cv2.imwrite(os.path.join(dir_path,ori_img.split("/")[-1]),outs)
...
with open('predict.txt', "r") as file:
    ...
    for name in img_files:
        ...
        try:
            test(ori_img = name,mask_img = mask_name,save_path = './results/'+clas, Net = UNet_Res, add_mask = True)
