...
def count_params(model, input_size=512,input_channel=3):
    # param_sum = 0
    with open('models.txt', 'w') as fm:
        fm.write(str(model))
 
    # 计算模型的计算量
    flops = calc_flops(model, input_size=input_size,input_channel=input_channel)
...
# 计算模型的计算量
def calc_flops(model, input_channel=3, input_size=512):
    ...
    if '0.4.' in torch.__version__:
        if assets.USE_GPU:
            input = torch.cuda.FloatTensor(torch.rand(2, input_channel, input_size, input_size).cuda())
        else:
            input = torch.FloatTensor(torch.rand(2, input_channel, input_size, input_size))
    else:
        input = Variable(torch.rand(2, input_channel, input_size, input_size), requires_grad=True)
    _ = model(input)
